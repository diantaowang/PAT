...
void PrintCall(Record CUST[])
{
	...
	for(i=0; i<NUM-1; i++)
	{
		...
		{
			if(flag == 0)
				printf("%s %02d\n", CUST[i].id, CUST[i].data[0]);
			if(CUST[i].status[1] == 'n' && CUST[i+1].status[1] == 'f')
			{
				time = (CUST[i+1].time - CUST[i].time);
				printf("%02d:%02d:%02d ", CUST[i].data[1], CUST[i].data[2], CUST[i].data[3]);
				printf("%02d:%02d:%02d ", CUST[i+1].data[1], CUST[i+1].data[2], CUST[i+1].data[3]);
				price = CalcCharge(CUST[i].data, CUST[i+1].data);
				charge += price;
				printf("%d $%.2lf\n", time, (double)price/100.0);
			}
			flag = 1;
		}
		else
...
int main()
{
	...
	for(i=0; i<N; i++)
	{
		...
		Cust.time = Cust.data[3] + 60*Cust.data[2] + 1440*Cust.data[1];
		RecordSort(&Cust);
	}	
	PrintCall(CUST);
//	for(i=0; i<NUM; i++)
//		printf("%s  %02d:%02d:%02d\n", CUST[i].id, CUST[i].data[1], CUST[i].data[2], CUST[i].data[3]);
	return 0;
}
